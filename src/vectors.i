...
// call this at the very end, to extend all custom-made vectors as well
%define EXTEND_VECTORS
...
for name, obj in g.iteritems():
  if name.startswith("Vector") and type(obj) == type:
    # SWIG generates __repr__ members for all classes, so we need to explicitly override them all
    obj.__repr__ = PythonicVector.__repr__

    # provide all other members at once through inheritance
    if PythonicVector not in obj.__bases__: # some classes are aliases to others
      obj.__bases__ += (PythonicVector,)
